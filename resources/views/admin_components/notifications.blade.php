...
<div id="notificationDetailModal" class="modal-overlay hidden">
    <div class="modal max-w-xl">
        <div id="notifDetailHeader" style="background: linear-gradient(135deg, #1E2A4A 0%, #25335A 100%); padding: 1.25rem 1.5rem;">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div style="width: 40px; height: 40px; background: rgba(255,255,255,0.15); border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                    ...
                    <div>
                        <h2 id="notifDetailTitle" class="text-xl font-bold" style="color: #fff; margin: 0;">Notification</h2>
                        <p id="notifDetailTime" style="margin: 4px 0 0 0; color: rgba(255,255,255,0.7); font-size: 12px;"></p>
                    </div>
                </div>
                <button onclick="closeModal('notificationDetailModal')" style="background: rgba(255,255,255,0.1); border: none; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                    <i data-lucide="x" class="w-5 h-5" style="color: #fff;"></i>
                </button>
            </div>
        </div>
        <div class="p-6">
            ...
            <div id="notifDetailFooter" class="mt-6 pt-4 border-t border-gray-100 flex items-center gap-3">
                <span id="notifDetailCategory" class="inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full"></span>
...
<div class="card mb-8">
    ...
    <div class="card border-l-4 border-l-red-500 overflow-hidden">
        ...
        <div class="divide-y divide-gray-100 max-h-80 overflow-y-auto">
            @forelse($important as $n)
                <div class="flex items-start gap-3 p-4 hover:bg-red-50/50 transition-colors group cursor-pointer" data-id="{{ $n->id }}" onclick="openNotificationDetail({{ $n->id }}, '{{ addslashes($n->title) }}', '{{ addslashes($n->message) }}', '{{ $n->created_at->format('M d, Y h:i A') }}', 'important', {{ $n->is_read ? 'true' : 'false' }})">
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onclick="event.stopPropagation()">
                        <button onclick="event.stopPropagation(); toggleImportant({{ $n->id }})" class="p-1.5 hover:bg-red-100 rounded transition-colors" title="Unmark important">
                            <i data-lucide="star" class="w-4 h-4 text-red-500 fill-red-500"></i>
                        </button>
                        @if(!$n->is_read)
                        <button onclick="event.stopPropagation(); markAsRead({{ $n->id }})" class="p-1.5 hover:bg-green-100 rounded transition-colors" title="Mark as read">
        ...
        <div class="flex items-center justify-between p-4 border-b border-gray-100 bg-blue-50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $inboxTotal }} item{{ $inboxTotal !== 1 ? 's' : '' }}</p>
                </div>
            </div>
        ...
        <div class="divide-y divide-gray-100 max-h-80 overflow-y-auto">
            @forelse($inboxPaginated as $entry)
                @if($entry->type === 'notification')
                    ...
                    <div class="flex items-start gap-3 p-4 hover:bg-blue-50/50 transition-colors group cursor-pointer" data-id="{{ $n->id }}" onclick="openNotificationDetail({{ $n->id }}, '{{ addslashes($n->title) }}', '{{ addslashes($n->message) }}', '{{ $n->created_at->format('M d, Y h:i A') }}', 'inbox', {{ $n->is_read ? 'true' : 'false' }})">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onclick="event.stopPropagation()">
                            <button onclick="event.stopPropagation(); toggleImportant({{ $n->id }})" class="p-1.5 hover:bg-yellow-100 rounded transition-colors" title="{{ $n->is_important ? 'Unmark important' : 'Mark important' }}">
                ...
                @elseif($entry->type === 'resignation')
                    ...
                    <div class="flex items-start gap-3 p-4 hover:bg-blue-50/50 transition-colors group cursor-pointer" onclick="window.location.href='{{ route('dashboard.members') }}'">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-sm font-medium text-gray-900">{{ $r->user->first_name ?? '' }} {{ $r->user->last_name ?? '' }}</p>
                                <span class="text-xs text-gray-400 whitespace-nowrap">{{ $r->created_at->diffForHumans() }}</span>
                            </div>
    ...
    <div class="card border-l-4 border-l-orange-500 overflow-hidden">
        ...
        <div class="divide-y divide-gray-100 max-h-80 overflow-y-auto">
            @forelse($spam as $n)
                <div class="flex items-start gap-3 p-4 hover:bg-orange-50/50 transition-colors group cursor-pointer" data-id="{{ $n->id }}" onclick="openNotificationDetail({{ $n->id }}, '{{ addslashes($n->title) }}', '{{ addslashes($n->message) }}', '{{ $n->created_at->format('M d, Y h:i A') }}', 'spam', {{ $n->is_read ? 'true' : 'false' }})">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            <p class="text-sm font-medium {{ $n->is_read ? 'text-gray-500' : 'text-gray-900' }}">{{ $n->title }}</p>
                            <span class="text-xs text-gray-400 whitespace-nowrap">{{ $n->created_at->diffForHumans() }}</span>
    ...
    <div class="card border-l-4 border-l-purple-500 overflow-hidden">
        <div class="flex items-center justify-between p-4 border-b border-gray-100 bg-purple-50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $socialTotal }} item{{ $socialTotal !== 1 ? 's' : '' }}</p>
                </div>
            </div>
        ...
        <div class="divide-y divide-gray-100 max-h-80 overflow-y-auto">
            @forelse($socialPaginated as $entry)
                @if($entry->type === 'notification')
                    @php $n = $entry->item; @endphp
                    <div class="flex items-start gap-3 p-4 hover:bg-purple-50/50 transition-colors group cursor-pointer" data-id="{{ $n->id }}" onclick="openNotificationDetail({{ $n->id }}, '{{ addslashes($n->title) }}', '{{ addslashes($n->message) }}', '{{ $n->created_at->format('M d, Y h:i A') }}', 'social', {{ $n->is_read ? 'true' : 'false' }})">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-sm font-medium {{ $n->is_read ? 'text-gray-500' : 'text-gray-900' }}">{{ $n->title }}</p>
                                <span class="text-xs text-gray-400 whitespace-nowrap">{{ $n->created_at->diffForHumans() }}</span>
                ...
                @elseif($entry->type === 'comment')
                    ...
                    <div class="flex items-start gap-3 p-4 hover:bg-purple-50/50 transition-colors group cursor-pointer" onclick="scrollToAnnouncement({{ $c->announcement_id }})">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <span class="text-xs text-gray-400 whitespace-nowrap">{{ $c->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-xs text-gray-500 mt-0.5">Commented on <span class="font-medium text-gray-700">{{ $c->announcement->title ?? 'an announcement' }}</span></p>
                            <p class="text-xs text-gray-600 mt-1 bg-purple-50 rounded-lg px-3 py-2">{{ Str::limit($c->comment, 100) }}</p>
                        </div>
                    </div>
                @endif
            @empty
